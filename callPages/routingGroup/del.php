...
for ($i=0;$i<$total;$i++){
	...
	if ($nPhone != ""){
		...
		$totalCnt++;
		$delsql="delete from $bbs where CONCAT(RGROUP,':',PREFIX)='$nPhone'" ;
		if (mysqli_query($db,$delsql)) {
		   $aDelPhone[]=$strKey;
		   $arrPhone=explode(":",$nPhone );
		   $eventSql = "insert into EVENT_$bbs (O_RGROUP , O_PREFIX, X_ACTION) values ('$arrPhone[0]','$arrPhone[1]', 'D')"; 		   
		   mysqli_query($db,$eventSql) ;	

		   $cnt++;
		   regLog($admin_info['user_id'], '3','del','Group-'.$arrPhone[0].' : '.$arrPhone[1] , '','ENG',$reg_date,'rout_group') ;
		} else {
			$aErrStaus[]=$strKey ;
		}
	}
