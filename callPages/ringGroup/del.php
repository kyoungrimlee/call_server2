...
for ($i=0;$i<$total;$i++){
	...
	if ($nPhone != ""){
		$totalCnt++;
		$delsql="delete from $bbs where RCODE='$nPhone'" ;
		if (mysqli_query($db,$delsql)) {
		    $aDelPhone[]=$nPhone;
		    $eventSql="insert into EVENT_$bbs (O_RCODE , X_ACTION) values ('$nPhone', 'D')" ; 
		    mysqli_query($db,$eventSql) ;	
			$cnt++;
			regLog($admin_info['user_id'], '8','del',"Ring Group - ".$nPhone , '','ENG',$reg_date,'etc_groupRring') ;
		} else {
			$aErrStaus[]=$nPhone;
		}
	}
