...
for ($i=0;$i<$total;$i++){
	...
	if ($nPhone != ""){
		$totalCnt++;
		$delsql="delete from $bbs where PCODE='$nPhone'" ;
		if (mysqli_query($db,$delsql)) {
		   $aDelPhone[]=$nPhone;
		   $eventSql="insert into EVENT_$bbs (O_PCODE , X_ACTION) values ('$nPhone', 'D')";
		    mysqli_query($db,$eventSql) ;	
			$cnt++;
			regLog($admin_info['user_id'], '8','del','Group - '.$nPhone , '','ENG',$reg_date,'etc_cpgroup') ;
		} else {
			$aErrStaus[]=$nPhone;
		}
	}
