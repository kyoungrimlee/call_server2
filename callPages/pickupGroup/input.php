<? include_once dirname(__FILE__) . "/../../lib/setConfig.php";
	...
	$alevel=array("view"=>$admin_info[$levelKey.'_view'], "add"=>$admin_info[$levelKey.'_add'], "mod"=>$admin_info[$levelKey.'_mod'], "del"=>$admin_info[$levelKey.'_del']);
  	...
  	if ($key) {
		$sql="select * from $bbs where PCODE='$key'";
...
<input type=hidden name="old_key" id='old_key' value="<?=$old_key?>"  >
	...
	<dl class="layer_box_wrap on">
		<dt><?=$tit['mainTitle']['etc_cpgroup']?><span class="btn_close_container" onclick="LayerPopup_type2('close');return false">닫기</span></dt>
		<dd>
			<table class="bbs_table_vertical" cellpadding="0" cellspacing="0" border="0">
				...
				<tbody >
					<tr>
						<th><?=$titEtc['listTitle']['gCode']?></th>
						<td><input type="text" id="pcode" name="pcode" class="input_w170 intbox numOnly" value="<?=$row['PCODE']?>" onchange="requestChkInfo()" required label="<?=$titEtc['listTitle']['gCode']?>"></td>
					</tr>
					<tr>
						<th><?=$titEtc['listTitle']['gName']?></th>
						<td><input type="text" id="pname" name="pname" class="input_w170" value="<?=$row['PNAME']?>"></td>
					</tr>
