...
for ($i=0;$i<$total;$i++){
	...
	if ($nPhone != ""){
		$totalCnt++;
		$delsql="delete from $bbs where IPADDR='$nPhone'" ;
		if (mysqli_query($db,$delsql)) {
		   $aDelPhone[]=$nPhone;
		   $eventSql="insert into EVENT_$bbs (O_IPADDR , X_ACTION) values ('$nPhone', 'D')";
		    mysqli_query($db,$eventSql) ;	
			$cnt++;
			regLog($admin_info['user_id'], '6','del','IP Address - '.$nPhone , '','ENG',$reg_date,'pri') ;
		} else {
			$aErrStaus[]=$nPhone;
		}
	}
