<? include_once dirname(__FILE__) . "/../../lib/setConfig.php"; 
	...
	$alevel=array("view"=>$admin_info[$levelKey.'_view'], "add"=>$admin_info[$levelKey.'_add'], "mod"=>$admin_info[$levelKey.'_mod'], "del"=>$admin_info[$levelKey.'_del']);
  	if ($key) {
		$sql="select * from $bbs where CONCAT(PHONE,':',SOURCE)='$key'";
...
<input type=hidden name="old_key2" id='old_key2' value="<?=$old_key2?>"  >
	...
	<dl class="layer_box_wrap on">
		<dt><?=$tit['mainTitle']['digit_group']?><span class="btn_close_container" onclick="LayerPopup_type2('close');return false">닫기</span></dt>
		<dd>
			<table class="bbs_table_vertical" cellpadding="0" cellspacing="0" border="0">
				...
				<tbody >
					<tr>
						<th><?=$titDigit['listTitle']['phone']?></th>
						<td><input type="text" id="phone" name="phone" class="input_w170 numOnly" value="<?=$row['PHONE']?>"  required label="<?=$titDigit['listTitle']['phone']?>"></td>
					</tr>				
					<tr>
