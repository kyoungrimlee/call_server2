...
class digitGroup Extends Page {
	...
	function get_ListValue() {
		...
		while ($data = mysqli_fetch_array($res)){
			...
			//체크여부처리
			$arrChkOk= array_search($data['PHONE'].":".$data['SOURCE'],$arrChk);
			if ($arrChkOk > 0) {
				$checked="checked";
			}else {
				$checked="";		
			...
			echo "<tr id='$data[PHONE]:$data[SOURCE]'  $bg >";
			if ($alevel['del']=="Y") {
			   echo "<td class='notList'><input type='checkbox' name='chk[]' value='$data[PHONE]:$data[SOURCE]' onclick='selchk(this)' $checked></td>";
			}

			echo"<td >$num</td>
				<td class='ta_left pl_30'><span class='fc_black'>".$data['PHONE']."</span></td>
				<td class='fc_red2'>".$data['SOURCE']."</td>
			    <td >".$data['TARGET']."</td>
			</tr>";
