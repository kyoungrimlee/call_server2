<?
class routingGroup Extends Page {
	...
	function get_ListValue() {
		...
		while ($data = mysqli_fetch_array($res)){
			...
			//체크여부처리
			$arrChkOk= array_search($data['RGROUP'].":".$data['PREFIX'],$arrChk);
			if ($arrChkOk > 0) {
				$checked="checked";
			}else {
				$checked="";		
			...
			echo "<tr id='$data[RGROUP]:$data[PREFIX]'  $bg >";
			if ($alevel['del']=="Y") {
			   echo "<td class='notList'><input type='checkbox' name='chk[]' value='$data[RGROUP]:$data[PREFIX]' onclick='selchk(this)' $checked></td>";
			}

			echo"<td >$num</td>
				<td  class='ta_left pl_30'><span class='fc_black'>".$data['RGROUP']."</span></td>
				<td  class='ta_left pl_30 fc_red2'>".$data['PREFIX']."</td>
				<td  class='ta_left pl_30 fc_purple'>".$data['ROUTING_IP']."</td>
			    <td >".$data['PORT']."</td>
