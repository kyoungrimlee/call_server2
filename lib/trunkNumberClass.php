<?
class trunkNumber Extends Page {
		...
		foreach($_GET as $_tmp['k'] => $_tmp['v']) {
 	    ...
 	    if (isset($_GET['word'])==true && $_GET['word']!="") {
		  ...
		  if ($find) {
			  ...
			  //검색조건이 전체일때 통합검색
			  $where[] ="TNUMBER like '%$word%'";
			  $where[] ="TNAME  like '%$word%'";
	...
	function get_ListValue() {
		...
		while ($data = mysqli_fetch_array($res)){
			...
			//체크여부처리
			$arrChkOk= array_search(trim($data['TNUMBER']),$arrChk);
			if ($arrChkOk > 0) {
				$checked="checked";
			}else {
				$checked="";		
			...
			echo "<tr id='$data[TNUMBER]'  $bg >";
			if ($alevel['del']=="Y") {
			   echo "<td class='notList'><input type='checkbox' name='chk[]' value='".$data['TNUMBER']."' onclick='selchk(this)' $checked></td>";
			}

			echo"<td >$num</td>
				<td class='ta_left pl_30'><span class='fc_black'>".$data['TNUMBER']."</span></td>
				<td class='ta_left pl_30 fc_blue2'>".$data['TNAME']."</td>
				<td >".$data['TPORT']."</td>
				<td class='fc_red2'>".$data['RCODE']."</td>
			</tr>";
