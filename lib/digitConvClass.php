...
class digitConv Extends Page {
	...
	function get_ListValue() {
		...
		while ($data = mysqli_fetch_array($res)){
			...
			//체크여부처리
			$arrChkOk= array_search(trim($data['SOURCE']),$arrChk);
			if ($arrChkOk > 0) {
				$checked="checked";
			}else {
				$checked="";		
			...
			echo "<tr id='$data[SOURCE]'  $bg >";
			if ($alevel['del']=="Y") {
			   echo "<td class='notList'><input type='checkbox' name='chk[]' value='".$data['SOURCE']."' onclick='selchk(this)' $checked></td>";
			}

			echo"<td >$num</td>
				<td ><span class='fc_black'>".$data['SOURCE']."</span></td>
				<td >".$data['TARGET']."</td>
			</tr>";
